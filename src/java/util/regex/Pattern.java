...
public final class Pattern implements Serializable {
     ...
     */
    private Pattern(String p, int flag) {
        // flag必须是ALL_FLAGS以内的标记，或者为0
        if((flag & ~ALL_FLAGS) != 0) {
            throw new IllegalArgumentException("Unknown flag 0x" + Integer.toHexString(flag));
        }
     ...
     */
    /*
     * 将给定的输入input从当前正则表达式匹配的地方切割，并返回切割后的子串集合（忽略结尾空串）
     * 如果正则表达式能够匹配文本的最开头部分，那么返回数组的第一个元素应该为空串
     * 如果正则表达式能够在某个位置匹配两次以上，则该处也会返回空串，因为邻近的匹配"分割"了零长度的文本
     */
     ...
     */
    /*
     * 将给定的输入input从当前正则表达式匹配的地方切割，并返回切割后的子串集合
     *
     * 如果正则表达式能够匹配文本的最开头部分，那么返回数组的第一个元素应该为空串
     * 如果正则表达式能够在某个位置匹配两次以上，则该处也会返回空串，因为邻近的匹配"分割"了零长度的文本
     *
     * 该版本的split能够控制pattern应用的次数，以及决定结尾部分出现空元素时的处理策略
     *
     * limit<0：保留数组结尾的空元素
     * limit==0：不保留结尾的空元素，效果与单参数的split一致
     * limit>0，保留前limit个子串
     */
     ...
     * @since 1.8
     */
    // 使用当前Pattern中的正则表达式，将input分割后，存入流中并返回
     ...
     *
     * @apiNote This method creates a predicate that behaves as if it creates a matcher
     ...
     * @since 1.8
     */
    // 基于当前Pattern中的正则表达式，将Matcher的find()包装成一个Predicate
     ...
     * @since 11
     */
    // 基于当前Pattern中的正则表达式，将Matcher的matches()包装成一个Predicate
    ...
    static BmpCharPredicate HorizWS() {
        return cp -> cp == 0x09 || cp == 0x20 || cp == 0xa0 || cp == 0x1680 || cp == 0x180e || cp >= 0x2000 && cp<=0x200a || cp == 0x202f || cp == 0x205f || cp == 0x3000;
    }
    ...
    static CharPredicate Range(int lower, int upper) {
        if(upper<Character.MIN_HIGH_SURROGATE || lower>Character.MAX_HIGH_SURROGATE && upper<Character.MIN_SUPPLEMENTARY_CODE_POINT)
    ...
    static CharPredicate CIRange(int lower, int upper) {
        return ch -> inRange(lower, ch, upper) || ASCII.isAscii(ch) && (inRange(lower, ASCII.toUpper(ch), upper) || inRange(lower, ASCII.toLower(ch), upper));
    }
...
loop:
     ...
     * 返回seq中包含的码点数量
     *
     * 能组成完整Unicode符号的一对char算作一个码点
     * 对于某个异常码点，如只有高代理或只有低代理的char，也会算作一个码点
     * 其他正常的基本平面区的码点，都是一个char对应一个码点
     */
