...
public abstract class Calendar implements Serializable, Cloneable, Comparable<Calendar> {
     ...
     * @serial
     */
    private int minimalDaysInFirstWeek; // 一年中第一周的第一天最小允许是周几
     ...
     */
    private static final int UNSET = 0;    // 用在stamp[]中，代表某个Calendar字段未设置
    
    /**
     * The value of the corresponding fields[] has been calculated internally.
     */
    private static final int COMPUTED = 1; // 用在stamp[]中，代表某个Calendar字段已设置
    ...
    public void set(int field, int value) {
        ...
        // 如果fields属性和time属性已经同步，但是fields属性中存在未设置的字段，则此时只需要更新fields
        if(areFieldsSet && !areAllFieldsSet) {
            // 将time属性中的纪元毫秒转换为fields[]中的日期-时间字段值
            computeFields();
        }
        ...
        isTimeSet = false;          // time属性开始无效
        areFieldsSet = false;       // fields属性与time属性失去同步
        isSet[field] = true;        // 该字段已设置
        stamp[field] = nextStamp++; // 记下时间戳
     ...
     */
    /*
     * 在Calendar字段field的原值基础上，增/减amount个单位。
     * 这个修改是会牵连其他字段的，比如从3月1日往前调一天，那么这里将显示2月28日或2月29日。
     */
     ...
     */
    /*
     * 在Calendar字段field的原值基础上，增/减一个单位；up用来指示是增(true)还是减(false)。
     * 这个修改是会牵连其他字段的，比如从3月1日往前调一天，那么这里将显示3月31日。
     */
    ...
    /*
     * 在Calendar字段field的原值基础上，增/减amount个单位。
     * 这个修改不会牵连其他字段，比如从3月1日往前调一天，那么这里将显示3月31日。
     */
     ...
     * @see #getActualMaximum(int)
     */
    // 返回指定字段上区间的最大值，比如Calendar.DAY_OF_MONTH字段返回31，即一月最多31天
     ...
     * @see #getActualMaximum(int)
     */
    // 返回指定字段上区间的最小值，比如Calendar.DAY_OF_MONTH字段返回28，即一月最少28天
     ...
     * @since 1.2
     */
    // 返回指定字段在当前上下文中的最大值，比如Calendar.DAY_OF_MONTH字段在1月返回31,2月返回28/29,4月返回30
    ...
    public long getTimeInMillis() {
        // 如果time属性已无效，则需要更新它
        if(!isTimeSet) {
            // 将time属性中的纪元毫秒与fields[]中的日期-时间字段值更新为一致
            updateTime();
        }
    ...
    public void setTimeInMillis(long millis) {
        // If we don't need to recalculate the calendar field values, do nothing.
    ...
    public TimeZone getTimeZone() {
        // If the TimeZone object is shared by other Calendar instances, then create a clone.
        if(sharedZone) {
     ...
     */
    /*
     * 返回某个字段在指定区域下的显示风格；style指示使用长格式还是短格式
     *
     * 举例：
     * getDisplayName(Calendar.MONTH, Calendar.SHORT, Locale.CHINA) // 12月
     * getDisplayName(Calendar.MONTH, Calendar.LONG, Locale.CHINA)  // 十二月
     */
     ...
     * @since 1.6
     */
    // 返回某个字段在指定区域下的所有显示风格；style指示使用长格式还是短格式
     ...
     * @see #computeFields()
     */
    // 将fields[]中的日期-时间字段值转换为time属性中的纪元毫秒
     ...
     */
    // 将time属性和fields属性更新为一致
    protected void complete() {
        // 如果time属性已无效，则需要更新time
        if(!isTimeSet) {
            // 将time属性中的纪元毫秒与fields[]中的日期-时间字段值更新为一致
            updateTime();
        }
        
        // 如果fields属性与time属性不同步，或者fields属性中存在未设置的字段，则需要更新fields
        if(!areFieldsSet || !areAllFieldsSet) {
            // 将time属性中的纪元毫秒转换为fields[]中的日期-时间字段值
    ...
    private static TimeZone defaultTimeZone(Locale l) {
        ...
        return TimeZoneNameUtility.convertLDMLShortID(shortTZID).map(TimeZone::getTimeZone).orElse(defaultTZ);
    }
     ...
     * nextStamp is set to the next stamp value upon the return.
     */
    // 调整伪时间戳
    ...
    private void invalidateWeekFields() {
        // 只有在这两个字段都设置完的情形下才进行后续处理
        if(stamp[WEEK_OF_MONTH] != COMPUTED && stamp[WEEK_OF_YEAR] != COMPUTED) {
            return;
        }
        
        /*
         * We have to check the new values of these fields after changing firstDayOfWeek and/or minimalDaysInFirstWeek.
         * If the field values have been changed, then set the new values. (4822110)
         */
    ...
    public static class Builder {
        ...
        private Locale locale;  // 区域信息
        private long instant;   // 纪元毫秒
        private TimeZone zone;  // 时区信息
        
        private String type;    // 历法类型
        ...
        public Calendar build() {
            ...
            switch(type) {
                ...
                case "iso8601":
                    ...
                    // 设置儒略历转换为公历的时间点，默认值为1582年10月15日。在ISO历法中，使用的是Proleptic年，不存在这个转换问题。
                    gcal.setGregorianChange(new Date(Long.MIN_VALUE));
                    /* and week definition to be compatible with ISO 8601 */
                    // 在ISO历法中，规定一周第一天是周一，且一年中第一个周四所在的周是一年中的第一周
            ...
            // 如果已经为Builder设置了纪元毫秒，则将其复制到Calendar中，并标记构造完成
            if(isInstantSet()) {
                // 为Calendar设置纪元毫秒
            ...
            // Set the fields from the min stamp to the max stamp so that the fields resolution works in the Calendar.
        ...
        public Builder setInstant(long instant) {
            ...
            this.nextStamp = COMPUTED;
            return this;
        }
        
        /**
         * Sets the date parameter to the {@code instant} value given by a {@link Date}.
         * This method is equivalent to a call to {@link #setInstant(long) setInstant(date.getTime())}.
         *
         * @param date a {@code Date} representing a millisecond offset from the Epoch
         *
         * @return this {@code Calendar.Builder}
         *
         ...
         * @see Calendar#time
         */
        // 将date中的纪元毫秒设置到Builder中
        public Builder setInstant(Date date) {
            // 获取Date内部的纪元毫秒
            long instant = date.getTime();
            return setInstant(instant); // NPE if instant == null
        }
         ...
         * {@linkplain #setLocale(Locale) locale}.
         *
         * @param firstDayOfWeek         the first day of a week; one of {@link Calendar#SUNDAY} to {@link Calendar#SATURDAY}
