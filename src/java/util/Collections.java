...
nextCand:
     ...
     */
    // 将已有的Set包装为同步Set(线程安全)，兼容SortedSet接口
     ...
     * @since 1.8
     */
    // 将已有的Set包装为同步Set(线程安全)，兼容SortedSet接口和NavigableSet接口
     ...
     */
    // 将已有的Map包装为同步Map(线程安全)，兼容SortedSet接口
     ...
     * @since 1.8
     */
    // 将已有的Map包装为同步Map(线程安全)，兼容SortedSet接口和NavigableSet接口
    ...
    public static <T> Comparator<T> reverseOrder(Comparator<T> cmp) {
        // 如果没有提供外部比较器，则使用内部比较器
        if(cmp == null) {
            return (Comparator<T>) ReverseComparator.REVERSE_ORDER;
        }
        
        if(cmp == ReverseComparator.REVERSE_ORDER) {
            // 返回自然顺序比较器，用于比较实现了Comparable的对象
     ...
     * This provides interoperability with legacy APIs that require an enumeration as input.
     *
     ...
     * More formally, returns the number of elements {@code e} in the collection such that {@code Objects.equals(o, e)}.
     *
     ...
     * @serial include
     */
    // 同步Set(线程安全)，兼容SortedSet接口和NavigableSet接口
     ...
     * @serial include
     */
    // 同步Map(线程安全)，兼容SortedSet接口和NavigableSet接口
    ...
    private static class UnmodifiableMap<K, V> implements Map<K, V>, Serializable {
        ...
        static class UnmodifiableEntrySet<K, V> extends UnmodifiableSet<Map.Entry<K, V>> {
            ...
            @SuppressWarnings("unchecked")
            public <T> T[] toArray(T[] a) {
                // We don't pass a to c.toArray, to avoid window of vulnerability wherein an unscrupulous multithreaded client could get his hands on raw (unwrapped) Entries from c.
    ...
    static class CheckedCollection<E> implements Collection<E>, Serializable {
        ...
        public boolean addAll(Collection<? extends E> coll) {
            // Doing things this way insulates us from concurrent changes in the contents of coll and provides all-or-nothing semantics
            // (which we wouldn't get if we type-checked each element as we added it)
        ...
        public Iterator<E> iterator() {
            // JDK-6363904 - unwrapped iterator could be typecast to ListIterator with unsafe set()
        ...
        static class CheckedEntrySet<K, V> implements Set<Map.Entry<K, V>> {
            ...
            public Object[] toArray() {
                ...
                for(int i = 0; i<source.length; i++) {
                    dest[i] = checkedEntry((Entry<K, V>) source[i], valueType);
                }
            ...
            public <T> T[] toArray(T[] a) {
                ...
                for(int i = 0; i<arr.length; i++) {
                    arr[i] = (T) checkedEntry((Entry<K, V>) arr[i], valueType);
                }
