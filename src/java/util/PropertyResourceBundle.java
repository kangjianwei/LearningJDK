 ...
 * </blockquote>
 *
 * @apiNote {@code PropertyResourceBundle} can be constructed either
 ...
 * @implSpec The implementation of a {@code PropertyResourceBundle} subclass must be
...
public class PropertyResourceBundle extends ResourceBundle {
    ...
    public PropertyResourceBundle(InputStream stream) throws IOException {
        this(new InputStreamReader(stream,  // 根据输入流和解码器创建Reader
                "ISO-8859-1".equals(encoding)
                    ? StandardCharsets.ISO_8859_1.newDecoder()  // 如果预设了ISO-8859-1字符集，就使用ISO-8859-1解码器
                    : new PropertyResourceBundleCharset("UTF-8".equals(encoding)).newDecoder()  // 否则使用属性资源解码器
            )
        );
    }
    ...
    public PropertyResourceBundle(Reader reader) throws IOException {
        ...
        properties.load(reader);    // 从properties文件加载属性集
        lookup = new HashMap(properties);   // 缓存资源文件的属性集
    }
