...
public interface IntStream extends BaseStream<Integer, IntStream> {
    ...
    static IntStream generate(IntSupplier supplier) {
        Objects.requireNonNull(supplier);
        
        return StreamSupport.intStream(new StreamSpliterators.InfiniteSupplyingSpliterator.OfInt(Long.MAX_VALUE, supplier), false);
    }
     ...
     */
    // 构造一个由s1和s2拼接而成的流
    static IntStream concat(IntStream s1, IntStream s2) {
        Objects.requireNonNull(s1);
        Objects.requireNonNull(s2);
        
        Spliterator.OfInt split = new Streams.ConcatSpliterator.OfInt(s1.spliterator(), s2.spliterator());
        IntStream stream = StreamSupport.intStream(split, s1.isParallel() || s2.isParallel());
