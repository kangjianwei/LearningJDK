...
public final class Scanner implements Iterator<String>, Closeable {
     ...
     * @throws IllegalArgumentException if the radix is out of range
     */
    // 判断下一个元素是否为BigInteger值，进制由参数radix指定
     ...
     * @throws IllegalArgumentException if the radix is out of range
     */
    // 如果下一个元素是radix进制的BigInteger值，则返回它
    ...
    public String findInLine(Pattern pattern) {
        ...
        /*
         * If there is nothing between the current pos and the next newline simply return null,
         * invoking findWithinHorizon with "horizon=0" will scan beyond the line bound.
         */
        if(horizonForLine == 0) {
            return null;
        }
     ...
     */
    // 返回下一个与正则模式pattern匹配的字符串，需要在上次匹配串后面的horizon个符号内搜索
     ...
     * @throws IllegalArgumentException if horizon is negative
     */
    // 返回下一个与正则模式pattern匹配的字符串，需要在上次匹配串后的horizon个符号内搜索
    ...
    /** After this method is called there will either be an exception or else there will be space in the buffer */
    ...
    /** When a buffer compaction/reallocation occurs the saved indexes must be modified appropriately. */
    ...
    /** The next operation should occur in the specified radix but the default is left untouched. */
